...
public:
  ...
  ~SSD1306(){};
  SYSHAT::ICommInterface &i2c_driver;
  ...
  OLED_Return_Codes_e OLEDBitmap(int16_t x, int16_t y, int16_t w, int16_t h,
                                 const uint8_t *data, bool invert);

  void OLEDbegin(uint8_t I2c_address = SSD1306_ADDR, bool I2C_debug = false);
  bool OLEDSetBufferPtr(uint8_t width, uint8_t height, uint8_t *pBuffer,
...
private:
  ...
  // uint16_t _I2C_speed =
  //     BCM2835_I2C_CLOCK_DIVIDER_626;   /**< Speed of I2C bus interface */
  ...
  uint16_t _I2C_ErrorDelay =
      100; /**<I2C delay(in between retry attempts) in event of error in mS*/
  uint8_t _I2C_ErrorRetryNum =
      3; /**< In event of I2C error number of retry attempts*/
  uint8_t _I2C_ErrorFlag = 0x00; /**< In event of I2C error holds bcm2835 I2C
  ...
  uint8_t _OLED_PAGE_NUM =
      (_OLED_HEIGHT /
       8); /**< Number of byte size pages OLED screen is divided into */

  uint8_t *OLEDbuffer =
      nullptr; /**< pointer to buffer which holds screen data */
};
