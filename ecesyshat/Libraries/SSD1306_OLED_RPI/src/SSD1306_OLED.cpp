...
SSD1306::SSD1306(int16_t oledwidth, int16_t oledheight,
                 SYSHAT::ICommInterface &i2c_obj)
    : SSD1306_graphics(oledwidth, oledheight), i2c_driver(i2c_obj) {
...
/*!
        ...
        @param I2C_speed default=0 , 0=bcm2835_i2c_set_baudrate(100000) 100k
   baudrate, >0 = BCM2835_I2C_CLOCK_DIVIDER,choices=2500 ,622 ,150 ,148
...
*/
bool SSD1306::OLEDSetBufferPtr(uint8_t width, uint8_t height, uint8_t *pBuffer,
                               uint16_t sizeOfBuffer) {
  if (sizeOfBuffer != width * (height / 8)) {
    printf("Error OLEDSetBufferPtr 1: buffer size does not equal : width * "
           "(height/8))\n");
    return false;
  }
  OLEDbuffer = pBuffer;
  if (OLEDbuffer == nullptr) {
    printf("Error OLEDSetBufferPtr 2: Problem assigning buffer pointer, not a "
           "valid pointer object\r\n");
    return false;
  }
  return true;
}
...
/*!
        @brief Called from OLEDbegin carries out Power on sequence and register
   init
*/
void SSD1306::OLEDinit() {

  usleep(1000 * SSD1306_INITDELAY);
...
/*!
        ...
        @param dataPattern can be set to zero to clear screen (not buffer) range
   0x00 to 0ff
...
/*!
        ...
        @param mydelay optional delay in milliseconds can be set to zero
   normally.
*/
void SSD1306::OLEDFillPage(uint8_t page_num, uint8_t dataPattern,
                           uint8_t mydelay) {
...
*/
OLED_Return_Codes_e SSD1306::OLEDBitmap(int16_t x, int16_t y, int16_t w,
  ...
  if (x > _width || y > _height) {
    printf(
        "Error drawBitmap 1: Bitmap co-ord out of bounds, check x and y\r\n");
    return OLED_BitmapScreenBounds;
  }
  // 2. bitmap weight and height
  if (w > _width || h > _height) {
    printf(
        "Error drawBitmap 2: Bitmap is larger than screen, check w and h\r\n");
  ...
  if (w % 8 != 0) {
    printf("Error drawBitmap 4: Bitmap width size is incorrect must be "
           "divisible evenly by 8: %u\r\n",
           w);
...
/*!
        ...
        @note In the event of an error will loop  _I2C_ErrorRetryNum times each
   time.with delay _I2C_ErrorDelay Printing the error code , see
   bcm2835I2CReasonCodes in bcm2835 docs.
*/
void SSD1306::I2C_Write_Byte(uint8_t value, uint8_t cmd) {
  ...
  ReasonCodes = i2c_driver.write(
      _I2C_address, reinterpret_cast<const uint8_t *>(ByteBuffer), 2);
  while (ReasonCodes !=
         0) { // failure to write I2C byte ,Error handling retransmit
...
void SSD1306::OLEDclearBuffer() {
  memset(this->OLEDbuffer, 0x00,
         (this->_OLED_WIDTH * (this->_OLED_HEIGHT / 8)));
}
...
*/
void SSD1306::OLEDBufferScreen(int16_t x, int16_t y, uint8_t w, uint8_t h,
                               uint8_t *data) {
  uint8_t tx, ty;
  uint16_t offset = 0;
...
/*!
        ...
        @details bcm2835I2Creasoncode.
                ...
                -# BCM2835_I2C_REASON_ERROR_CLKT    = 0x02,Received Clock
   Stretch Timeout
                -# BCM2835_I2C_REASON_ERROR_DATA    = 0x04, Not all data is sent
   / receive
                -# BCM2835_I2C_REASON_ERROR_TIMEOUT = 0x08 Time out occurred
   during sending
...
/*!
         @brief Gets the I2C error retry attempts, used in the event of an I2C
   write error
...
/*!
         @brief Sets the I2C error retry attempts used in the event of an I2C
   write error
         @details Number of times to retry in event of an error
...
uint8_t SSD1306::OLEDCheckConnection(void) {
  ...
  _I2C_ErrorFlag =
      i2c_driver.read(_I2C_address, reinterpret_cast<uint8_t *>(rxdata),
                      1); // returns reason code , 0 success
